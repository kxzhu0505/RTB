...
int main()
{
  ...
  geo_redraw_vd vob;
  geo_scene sc1 = geowin_new_scene(gw, (geo_update_base<list<POINT>, list<POINT> >&)vob, (geo_redraw_base&)vob, sc_input, string("VORONOI")); 
  gw.set_color(sc1,blue);
  gw.set_visible(sc1,true);

  geo_scene sc2 = geowin_new_scene(gw, f_voronoi,        sc_input, string("F_VORONOI"),H1,H1);
  SET_REDRAW(sc2,f_voro_redraw,POINT);
  gw.set_color(sc2,green);

  geo_scene sc3 = geowin_new_scene(gw, delaunay_triang,  sc_input, string("DELAUNAY"),H1,H1);
  SET_REDRAW(sc3,delau_redraw,POINT);
  gw.set_color(sc3,orange);

  geo_scene sc4 = geowin_new_scene(gw, f_delaunay_triang,sc_input, string("F_DELAUNAY"),H1,H1);
  SET_REDRAW(sc4,f_delau_redraw,POINT);
  gw.set_color(sc4,red);

  geo_scene sc5 = geowin_new_scene(gw, min_spanning_tree,sc_input, string("MINSPAN_TREE"),H1,H1);
  SET_REDRAW(sc5,mst_redraw,POINT);
  gw.set_color(sc5,black);

  geo_scene sc6 = geowin_new_scene(gw, convex_hull,      sc_input, string("CONVEX HULL"),H1,H2);
  gw.set_color(sc6,grey1);

  geo_redraw_crust crs;
  geo_scene sc7 = geowin_new_scene(gw, (geo_update_base<list<POINT>, list<POINT> >&)crs, (geo_redraw_base&)crs, sc_input, string("Crust")); 
